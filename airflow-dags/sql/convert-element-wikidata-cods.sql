INSERT INTO oem.element_wikidata_cods (ew_el_id, ew_wikidata_cod, ew_from_name_etymology, ew_from_subject, ew_from_buried, ew_from_wikidata)
SELECT osm_id, UPPER(TRIM(wikidata_cod)), FALSE, FALSE, FALSE, TRUE
...
UNION
SELECT osm_id, UPPER(TRIM(subject_wikidata_cod)), FALSE, TRUE, FALSE, FALSE
...
UNION
SELECT osm_id, UPPER(TRIM(buried_wikidata_cod)), FALSE, FALSE, TRUE, FALSE
FROM oem.osmdata, LATERAL REGEXP_SPLIT_TO_TABLE(osm_tags->>'buried:wikidata',';') AS splitted(buried_wikidata_cod)
WHERE osm_tags ? 'buried:wikidata'
AND TRIM(buried_wikidata_cod) ~* '^Q\d+$'
UNION
SELECT osm_id, UPPER(TRIM(name_etymology_wikidata_cod)), TRUE, FALSE, FALSE, FALSE
