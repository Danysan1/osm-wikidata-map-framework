...
if [ "$(psql -h "$2" -U osm -d osm -t -c "SELECT COUNT(*) FROM wikidata WHERE wd_download_date IS NULL" | xargs)" == '0' ]; then
    ...
    WIKIDATA_IDS=$(psql -h "$2" -d osm -U osm -t -c "SELECT STRING_AGG('wd:'||wd_wikidata_id, ' ') FROM (SELECT wd_wikidata_id FROM wikidata WHERE wd_download_date IS NULL LIMIT 1000) AS x" | xargs)
...
if [ -f 'global-map.tmp.geojson' ]; then 
