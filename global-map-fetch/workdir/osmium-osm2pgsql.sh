...
if [ -f 'get_wikidata_ids.tmp.json' ]; then
    ...
    echo "s/__ELEMENTS__/$WIKIDATA_IDS/" > get_wikidata_ids.tmp.sed
...
if [ $(psql -h "$2" -U osm -d osm -t -c "SELECT COUNT(*) FROM wikidata_named_after" | xargs) != '0' ]; then
    ...
    echo "s|__WIKIDATA_JSON__|$WIKIDATA_JSON|" > load_wikidata_ids.tmp.sed
...
if [ -f 'get_wikidata_base.tmp.json' ]; then 
    ...
    echo "s/__WIKIDATA_IDS__/$WIKIDATA_IDS/" > get_wikidata_base.tmp.sed
...
if [ $(psql -h "$2" -U osm -d osm -t -c "SELECT COUNT(wd_position) FROM wikidata" | xargs) != '0' ]; then
    ...
    echo "s|__WIKIDATA_JSON__|$WIKIDATA_JSON|" > load_wikidata_base.tmp.sed
