...
class EtymologyColorControl extends DropdownControl {
    ...
    constructor(
        ...
        super(
            ...
            true,
            undefined,
            () => this.value = getColorSchemeID() ?? startColorScheme,
    ...
    private calculateAndLoadChartData(
        ...
        const colorSchemeIDChanged = this.lastColorSchemeID !== colorSchemeID,
            ...
            backgroundStyleID = getBackgroundStyleID(),
            backEndChanged = this.lastBackEndID !== backEndID,
            backgroundChanged = this.lastBackground !== backgroundStyleID;

        if (colorSchemeIDChanged || backEndChanged || backgroundChanged) {
            if (process.env.NODE_ENV === 'development') console.debug("calculateAndLoadChartData: updating layer color", { colorSchemeID, colorSchemeIDChanged, backEndID, backgroundStyleID, backEndChanged, layerColor });
            this.setLayerColor(layerColor);
        } else {
            if (process.env.NODE_ENV === 'development') console.debug("calculateAndLoadChartData: skipping layer color update", { colorSchemeID, colorSchemeIDChanged, backEndID, backgroundStyleID, backEndChanged });
        }
        ...
        if (colorSchemeIDChanged || backEndChanged || backgroundChanged || features?.length !== this.lastFeatureCount) {
            ...
            this.lastBackEndID = backEndID ?? undefined;
            this.lastBackground = backgroundStyleID ?? undefined;
            this.lastFeatureCount = features?.length;
            this.lastWikidataIDs = undefined;

            const stats = calculateChartData(features ?? []);
            if (process.env.NODE_ENV === 'development') console.debug("calculateAndLoadChartData: updating chart", { colorSchemeID, colorSchemeIDChanged, backEndID, backgroundStyleID, backEndChanged, features, stats });
    ...
    private async downloadChartDataForWikidataIDs(idSet: Set<string>, colorSchemeID: ColorSchemeID): Promise<EtymologyStat[] | null> {
        ...
        if (!colorSchemeChanged && !IDsChanged && !backEndChanged && !backgroundChanged) {
            if (process.env.NODE_ENV === 'development') console.debug("downloadChartDataForWikidataIDs: Skipping stats update for already downloaded IDs", { colorSchemeID, colorSchemeChanged, idSet, IDsChanged, backEndID, backgroundStyleID, backEndChanged });
            return null;
        }

        const uniqueIDs = Array.from(idSet);
        if (process.env.NODE_ENV === 'development') console.debug("downloadChartDataForWikidataIDs: Updating stats", { colorSchemeID, colorSchemeChanged, idSet, IDsChanged, backEndID, backgroundStyleID, backEndChanged });
        this.lastColorSchemeID = colorSchemeID;
