...
export const DetailsSourceAndLayers: React.FC<DetailsSourceAndLayersProps> = (props) => {
    ...
    useEffect(() => {
        ...
        if (process.env.NODE_ENV === "development") console.debug("DetailsSourceAndLayers useEffect", { bbox });
        if (bbox && props.backEndService?.canHandleBackEnd(props.backEndID)) {
            showLoadingSpinner(true);
            props.backEndService.fetchMapElements(props.backEndID, false, bbox, i18n.language).then(data => {
                setDetailsData(data);
            }).catch(e => {
