...
OSM-Wikidata Map Framework (OWMF) is a framework for creating interactive maps that combine OpenStreetMap and Wikidata data. It consists of two main components:
- **Front-end**: Next.js web application for map visualization (in `front-end/`)
- **Data pipeline**: Apache Airflow DAGs for generating PMTiles vector tiles (in `airflow/`)

## Common Commands

All commands run from `front-end/` directory:
...
For API client code generation (from `front-end/openapi/`):
```bash
