...
SELECT
    ...
    COUNT(*) FILTER (WHERE ety.et_recursion_depth = 0 AND ety.et_from_osm_wikidata_wd_id IS NULL AND ety.et_from_osm) AS count_osm,
    COUNT(*) FILTER (WHERE ety.et_recursion_depth = 0 AND ety.et_from_osm_wikidata_wd_id IS NOT NULL) AS count_osm_wikidata,
    COUNT(*) FILTER (WHERE ety.et_recursion_depth = 0 AND ety.et_from_osm_wikidata_wd_id IS NULL AND NOT ety.et_from_osm) AS count_wikidata,
