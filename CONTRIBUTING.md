...
The background map is provided by Maptiler, which is itself based on OpenStreetMap. You can improve the map on [openstreetmap.org](https://www.openstreetmap.org/).
You can learn how to map on [the official welcome page](https://www.openstreetmap.org/welcome) and on [LearnOSM](https://learnosm.org/). Keep in mind that Maptiler doesn't update the map immediately so if you edit something on OpenStreetMap it may take some time to appear in the map.
...
A template for this config file can be found in [`open-etymology-map.template.ini`](open-etymology-map.template.ini). When copying the template `.ini` you must set `maptiler_key`, while other options should already be ok as a starting point.
...
The map is created using [Maplibre GL JS](https://maplibre.org/maplibre-gl-js-docs/api/) and the charts are created using [chart.js](https://www.chartjs.org/).
