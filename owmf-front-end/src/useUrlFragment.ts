...
export function useUrlFragment() {
    const [lon, setLon] = useState<number>(() => process.env.owmf_default_center_lat ? parseFloat(process.env.owmf_default_center_lat) : 0),
        [lat, _setLat] = useState<number>(() => process.env.owmf_default_center_lon ? parseFloat(process.env.owmf_default_center_lon) : 0),
        [zoom, _setZoom] = useState<number>(() => process.env.owmf_default_zoom ? parseInt(process.env.owmf_default_zoom) : 1),
        [colorScheme, setColorScheme] = useState<ColorSchemeID>(() => { const raw = process.env.owmf_default_color_scheme; return raw && raw in ColorSchemeID ? raw as ColorSchemeID : ColorSchemeID.blue; }),
